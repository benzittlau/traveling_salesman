class Processor
  ...
  def self.sample
    SETS.each do |set|
      ...
      File.open("address_sets/#{set}_geocoded_sample.csv", "w+") do |f|
        f.puts(addresses.sample(20))
      end
    end
  end
  
  def self.build_matrixes
    SETS.each do |set|

      locations = CSV.read("address_sets/#{set}_geocoded_sample.csv", {:headers => true, :header_converters => :symbol}).map(&:to_hash)
      ...
      CSV.open("address_sets/#{set}_sample_matrix.csv",'w',
                  :write_headers => true,
